...
const PageObject = mergePages({}, PageBehavior, {
  ...
  getEntity: async function (actvtId) {
    ...
    try {
      ...
      if(result.data.languageType != '1') {
        this.setData({'pageLocale': 'en-US'})
      } else {
        this.setData({'pageLocale': 'zh-CN'})
      }
      this.changePageLanguage(this.data.pageLocale)

      let data = {}
      if (this.data.pageLocale == 'zh-CN') {
        data = {
          ...result.data,
  ...
  getUserSubmitData: async function (formid) {
    ...
    if (!this.data.formDataRowId) {
      ...
      if (result === null || !result.status) {
        ...
        if(result.returnData.formData && this.data.activity.onlineActvtUrl) {
          this.setData({
            onlineActvtUrl: this.data.activity.onlineActvtUrl
          })
        }
  ...
  goLiveStreamPage() {
    if(this.data.onlineActvtUrl) {
      wx.navigateTo({
        url: `/pages/webpage/webpage?webview_url=${encodeURIComponent(this.data.onlineActvtUrl)}`,
        fail: (err) => {
          wx.setClipboardData({
            data: this.data.onlineActvtUrl
          })
        }
      })
